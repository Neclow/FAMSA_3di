...
// *******************************************************************
CFAMSA::~CFAMSA() { delete final_profile; }
...
void CFAMSA::initScoreMatrix() {
    ...
    for (int i = 0; i < NO_AMINOACIDS; ++i) {
        ...
        for (int j = 0; j < NO_AMINOACIDS; ++j) score_matrix[i].emplace_back(SM_MIQS[i][j]);
    }
#else
    for (int i = 0; i < NO_AMINOACIDS; ++i) {
...
std::shared_ptr<AbstractTreeGenerator> CFAMSA::createTreeGenerator(const CParams& params) {
    ...
    if (params.export_distances) {
        ...
        if (params.distance == Distance::indel_div_lcs) {
            gen = make_shared<DistanceCalculator<Distance::indel_div_lcs>>(
                params.n_threads, params.instruction_set, params.output_file_name,
                params.generate_square_matrix, params.calculate_pid);
    ...
    if (params.gt_method == GT::SLINK ||
        (params.gt_heuristic != GT::None && params.gt_method == GT::MST_Prim)) {
        ...
        // UPGMA
        if (params.distance == Distance::indel_div_lcs) {
            gen = make_shared<UPGMA<Distance::indel_div_lcs>>(
                params.n_threads, params.instruction_set, (params.gt_method == GT::UPGMA_modified));
        } else if (params.distance == Distance::sqrt_indel_div_lcs) {
            gen = make_shared<UPGMA<Distance::sqrt_indel_div_lcs>>(
                params.n_threads, params.instruction_set, (params.gt_method == GT::UPGMA_modified));
        }
        ...
        if (params.distance == Distance::indel_div_lcs) {
            ...
            gen = make_shared<NeighborJoining<Distance::sqrt_indel_div_lcs>>(
    ...
    if (params.gt_heuristic != GT::None) {
        ...
        shared_ptr<IClustering> clustering =
            (params.gt_heuristic == GT::PartTree)
                ? nullptr
        ...
        if (params.distance == Distance::indel_div_lcs) {
            gen = make_shared<FastTree<Distance::indel_div_lcs>>(
                params.n_threads, params.instruction_set,
                dynamic_pointer_cast<IPartialGenerator>(gen), params.subtree_size, clustering,
                params.sample_size);
        } else if (params.distance == Distance::sqrt_indel_div_lcs) {
...
void CFAMSA::sortAndExtendSequences(std::vector<CSequence>& sequences) {
    ...
    std::transform(sequences.begin(), sequences.end(), seq_ptrs.begin(),
                   [](CSequence& s) { return &s; });

    auto comparer = [](const CSequence* a, const CSequence* b) -> bool {
        return a->length > b->length ||
               (a->length == b->length &&
                std::lexicographical_compare(a->data, a->data + a->data_size, b->data,
                                             b->data + b->data_size));
    };
...
void CFAMSA::extendSequences(std::vector<CSequence>& sequences) {
    // Temporarily resize the sequences by adding unknown symbols (just to simplify the pairwise
    // comparison)
    auto& max_seq =
        *max_element(sequences.begin(), sequences.end(),
...
// *******************************************************************
void CFAMSA::removeDuplicates(std::vector<CSequence*>& sorted_seqs,
                              std::vector<int>& original2sorted) {
...
CProfile* CFAMSA::ComputeAlignment(std::vector<CGappedSequence*>& gapped_sequences,
            ...
            while (pq.GetTask(prof_id, gs, prof1, prof2, no_threads, no_rows_per_box)) {
                if (gs != nullptr) {
                    ...
                    if (prof1->Size() + prof2->Size() > ref_thr) {
                        if (prof1->Size() <= ref_thr && prof1->Size() > 2) RefineAlignment(prof1);
                        if (prof2->Size() <= ref_thr && prof2->Size() > 2) RefineAlignment(prof2);
                    }
                ...
                if (params.very_verbose_mode) {
                    ...
                    if (computed_prof % 100 == 0 ||
                        (computed_prof % 10 == 0 &&
                         (double)computed_prof / (2 * gapped_sequences.size() - 1) > 0.95)) {
                        LOG_NORMAL << "Computing alignment - " << fixed << setprecision(1)
                                   << 100.0 * computed_prof / (2 * gapped_sequences.size() - 1)
                                   << "%    (" << computed_prof << " of "
                                   << (2 * gapped_sequences.size() - 1) << ")\r";
                        fflush(stdout);
                    }
                }
            }
...
bool CFAMSA::ComputeMSA(vector<CSequence>& sequences) {
    ...
    LOG_VERBOSE << "  distance measure: " << dist2str(params.distance) << "\n\n"
...
#ifdef DEVELOPER_MODE
    if (params.test_ref_sequences) LoadRefSequences();
#endif

    string instr_names[] = {"None",  "SSE",   "SSE2", "SSE3", "SSE3S",
                            "SSE41", "SSE42", "AVX",  "AVX2"};
    LOG_VERBOSE << "Hardware configuration: " << endl
                << " Number of threads: " << params.n_threads << endl
                << " Instruction set: " << instr_names[(int)instruction_set] << endl
                << endl;
    ...
    if (params.export_distances) {
        ...
        std::transform(sequences.begin(), sequences.end(), mapped_seqs.begin(),
                       [](CSequence& s) { return &s; });
        ...
        if (!params.keepDuplicates) {
            ...
            LOG_VERBOSE << mapped_seqs.size() << "/" << sequences.size() << " sequences retained."
                        << endl;
        }
        ...
        if (params.export_tree) {
            ...
            if (!RefineAlignment(final_profile)) return false;
            LOG_VERBOSE << "[OK]" << endl;
            ...
            std::vector<CGappedSequence*> ordered_unique_alignment(final_profile->data.size(),
                                                                   nullptr);
            for (int is = 0; is < (int)final_profile->data.size(); ++is) {
                ordered_unique_alignment[final_profile->data[is]->sequence_no] =
                    final_profile->data[is];
            }

...
#if SIMD == SIMD_AVX1 || SIMD == SIMD_AVX2 || SIMD == SIMD_AVX512
    if ((CPUID(1).ECX() >> 28) & 1) instruction_set = instruction_set_t::avx;
    if ((CPUID(7).EBX() >> 5) & 1) instruction_set = instruction_set_t::avx2;
#endif
...
void CParams::show_usage(bool expert) {
    ...
    LOG_NORMAL
        << "Usage:\n"
        << "  famsa3di [options] <input_file> [<input_file_2>] <output_file>\n\n"

        << "Positional parameters:\n"
        << "  input_file, input_file_2 - input files in FASTA format; action depends on the number "
           "of input files:\n"
        << "      * one input - multiple sequence alignment (input gaps, if present, are removed "
           "prior the alignment),\n"
        ...
        << "  output_file - output file (pass STDOUT when writing to standard output); available "
           "outputs:\n"
        ...
        << "  -medoidtree - use MedoidTree heuristic for speeding up tree construction (default: "
           "disabled)\n"
        << "  -medoid_threshold <n_seqs> - if specified, medoid trees are used only for sets with "
           "<n_seqs> or more\n"
        ...
        << "  -pid - generate pairwise identity (the number of matching residues divided by the "
           "shorter sequence length) instead of distance\n"
        << "  -keep-duplicates - keep duplicated sequences during alignment\n"
        << "                     (default: disabled - duplicates are removed prior and restored "
           "after the alignment).\n\n"

        << "  -gz - enable gzipped output (default: " << bool2str[gzippd_output] << ")\n"
        << "  -gz-lev <value> - gzip compression level [0-9] (default: " << gzip_level << ")\n"
        << "  -refine_mode <on | off | auto> - refinement mode (default: auto - the refinement is "
           "enabled for sets <= "
        << thr_refinement << " seq.)\n\n";
    ...
    if (expert) {
        ...
        LOG_NORMAL << "  -ref <file_name> - load referential sequences (for benchmarks) and "
                      "calculate the minimal subtree size containing them\n"
...
bool CParams::parse(int argc, char** argv, bool& showExpert) {
    ...
    if (findOption(params, "-gz-lev", g_lev) && (g_lev < 0 || g_lev > 12)) {
        LOG_NORMAL << "Incorrect gzip level: " << g_lev
                   << " was changed to default value: " << gzip_level << endl;
        g_lev = gzip_level;
    }
    gzip_level = g_lev;
